<?php
add_action( 'meni_el_after_main_css', 'notfound_style' );
function notfound_style() {
    if( is_404() ) {
        wp_enqueue_style( 'meni_el-404', get_theme_file_uri('/modules/404/assets/css/404.css'), false, MENI_EL_THEME_VERSION, 'all');
    }
}

add_filter( 'meni_el_add_inline_style', 'notfound_inline_style' );
function notfound_inline_style( $style ) {
    if( is_404() ) {
        $params = meni_el_404_page_params();
...
function meni_el_404_page_params() {
    $params = array(
        'enable_404message' => 1,
    ...
    return apply_filters( 'meni_el_404_page_params', $params );
}