...
<div class="entry-post-navigation"><?php
	...
	if( !empty( $prev_post ) ):	?>
			...
			<div class="nav-title-wrap">
				<h3><a href="<?php echo get_permalink( $prev_post->ID ); ?>" title="<?php echo esc_attr($prev_post->post_title); ?>"><?php
					if( get_the_title( $prev_post->ID ) == '') {
						echo esc_html__('Previous Post', 'meni-el');
					} else {
						echo "$prev_post->post_title";
					} ?></a>
				</h3>
				<p><a href="<?php echo get_permalink( $prev_post->ID ); ?>" title="<?php echo esc_attr($prev_post->post_title); ?>"><i class="wdticon-angle-left"></i><?php esc_html_e('Previous Story','meni-el'); ?></a></p>
		...
		<div class="post-prev-link no-post">
            <a href="#" style="background-image:url(<?php echo esc_url(MENI_EL_ROOT_URI.'/assets/images/no-post.jpg') ?>);" class="prev-post-bgimg"></a>
			<div class="nav-title-wrap">
				<span class="zmdi zmdi-long-arrow-left zmdi-hc-fw"></span>
				<h3><?php echo esc_html__('No previous story to show!', 'meni-el'); ?></h3>
			</div>
		</div>
		<?php
	endif;
	...
	if( !empty( $next_post ) ):	?>
			...
			<div class="nav-title-wrap">
				<h3><a href="<?php echo get_permalink( $next_post->ID ); ?>" title="<?php echo esc_attr($next_post->post_title); ?>"><?php
					if(get_the_title( $next_post->ID ) == '') {
						echo esc_html__('Next Post', 'meni-el');
					} else {
						echo "$next_post->post_title";
					} ?></a>
				</h3>
				<p><a href="<?php echo get_permalink( $next_post->ID ); ?>" title="<?php echo esc_attr($next_post->post_title); ?>"><?php esc_html_e('Next Story','meni-el'); ?><i class="wdticon-angle-right"></i></a></p>
		...
		<div class="post-next-link no-post">
            <a href="#" style="background-image:url(<?php echo esc_url(MENI_EL_ROOT_URI.'/assets/images/no-post.jpg') ?>);" class="next-post-bgimg"></a>
			<div class="nav-title-wrap">
				<span class="zmdi zmdi-long-arrow-right zmdi-hc-fw"></span>
				<h3><?php echo esc_html__('No next story to show!', 'meni-el'); ?></h3>
			</div>
		</div>
		<?php
	endif; ?>
