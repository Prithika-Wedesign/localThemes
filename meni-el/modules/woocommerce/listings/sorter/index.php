...
if( !class_exists( 'Meni_El_Woo_Listing_Sorter' ) ) {

    class Meni_El_Woo_Listing_Sorter {
        ...
        function __construct() {
            ...
            /* Sorter CSS */
                add_filter( 'meni_el_woo_archive_css', array( $this, 'woo_sorter_css'), 10, 1 );

            /* Sorter js */
                add_filter( 'meni_el_woo_archive_js', array( $this, 'woo_sorter_js'), 10, 1 );
        ...
        */
            ...
            function module_dir_path() {
                ...
                if( meni_el_is_file_in_theme( __FILE__ ) ) {
                    return MENI_EL_MODULE_DIR . '/woocommerce/listings/sorter/';
                } else {
            ...
            function module_dir_url() {
                ...
                if( meni_el_is_file_in_theme( __FILE__ ) ) {
                    return MENI_EL_MODULE_URI . '/woocommerce/listings/sorter/';
                } else {
        ...
        */
            function woo_sorter_settings() {
                ...
                $settings = apply_filters( 'meni_el_woo_sorter_settings', array () );
        ...
        */
            ...
            function woo_sorter_item_start_div() {
                add_filter( 'number_format_i18n', 'meni_el_pagination_zero_prefix' );
                ...
                if( is_shop() || is_product_category() || is_product_tag() ) {
                    ...
                    echo '<div class="product-layout-controller '.esc_attr($grid_controller_class).'">';
                        echo '<ul class="product-change-layout">';
                            echo '<li class="hidden"><span data-column="1" class="'.esc_attr($one_column_class).'">'.esc_html__('1', 'meni-el').'</span></li>';
                            echo '<li><span data-column="2" class="'.esc_attr($two_column_class).'">'.esc_html__('2', 'meni-el').'</span></li>';
                            echo '<li><span data-column="3" class="'.esc_attr($three_column_class).'">'.esc_html__('3', 'meni-el').'</span></li>';
                            echo '<li><span data-column="4" class="'.esc_attr($four_column_class).'">'.esc_html__('4', 'meni-el').'</span></li>';
                        echo '</ul>';
                    echo '</div>';
                    ...
                    echo '<div class="product-list-options-controller '.esc_attr($list_controller_class).'">';
                        echo '<ul class="product-list-options">';
                            echo '<li><span data-list-option="left-thumb" class="'.esc_attr($left_thumb_class).'">'.esc_html__('Left Thumb', 'meni-el').'</span></li>';
                            echo '<li><span data-list-option="right-thumb" class="'.esc_attr($right_thumb_class).'">'.esc_html__('Right Thumb', 'meni-el').'</span></li>';
                        echo '</ul>';
                    echo '</div>';
                ...
                if( is_shop() || is_product_category() || is_product_tag() ) {
                    ...
                    echo '<div class="product-display-controller">';
                        echo '<ul class="product-change-display">';
                            echo '<li><span data-display="grid" class="'.esc_attr($grid_display_mode_class).'">'.esc_html__('Grid', 'meni-el').'</span></li>';
                            echo '<li><span data-display="list" class="'.esc_attr($list_display_mode_class).'">'.esc_html__('List', 'meni-el').'</span></li>';
                        echo '</ul>';
                    echo '</div>';
            ...
            function woocommerce_pagination_args( $output ){
                ...
                $output = array(
                    'prev_text' => '<i class="wdticon-angle-left"></i><span>'. esc_html('Previous', 'meni-el') .'</span>',
                    'next_text' => '<i class="wdticon-angle-right"></i><span>'. esc_html('Next', 'meni-el') .'</span>',
                    'type'      => 'list',
                );
            ...
            function woo_filters_widget_area() {
                echo '<div class="product-loop-filters-area-group">';
                    echo '<div class="product-loop-filters-area-title">'.esc_html__('Filters', 'meni-el').'</div>';
                echo '</div>';
            }
            ...
            function woo_sorting_custom_extension() {
                echo '<div class="product-loop-filters-area-content hide">';
                    echo '<div class="product-loop-filters-area-content-inner">';
                        dynamic_sidebar( 'meni_el-shop-filters' );
                    echo '</div>';
                echo '</div>';
            }
...
if( !function_exists('meni_el_woo_listing_sorter') ) {
	function meni_el_woo_listing_sorter() {
		return Meni_El_Woo_Listing_Sorter::instance();
	}
}

meni_el_woo_listing_sorter();